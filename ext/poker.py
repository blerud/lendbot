...
def new_game(game: Game, message: discord.Message) -> List[str]:
    if game.state == GameState.NO_GAME:
        ...
        return [f"A new game has been started by {message.author.name}!", "Message `.p join` to join the game."]
    else:
        messages = ["There is already a game in progress, " "you can't start a new game."]
        if game.state == GameState.WAITING:
            messages.append("It still hasn't started yet, so you can still " "message `.p join` to join that game.")
        return messages
...
def join_game(game: Game, message: discord.Message) -> List[str]:
    if game.state == GameState.NO_GAME:
        return ["No game has been started yet for you to join.", "Message `.p newgame` to start a new game."]
        ...
        return [
            f"{message.author.name} has joined the game!",
            "Message `.p join` to join the game, " "or `.p start` to start the game.",
        ]
    else:
...
def deal_hand(game: Game, message: discord.Message) -> List[str]:
    if game.state == GameState.NO_GAME:
        return ["No game has been started for you to deal. " "Message `.p newgame` to start one."]
    elif game.state == GameState.WAITING:
        ...
        return [f"You aren't the dealer, {message.author.name}.", f"Please wait for {game.dealer.user.name} to `.p deal`."]
    else:
...
def check(game: Game, message: discord.Message) -> List[str]:
    if game.state == GameState.NO_GAME:
        ...
        return ["You can't check, because you're not playing, " f"{message.author.name}."]
    elif game.state == GameState.NO_HANDS:
        return ["You can't check because the hands haven't been dealt yet."]
    elif game.current_player.user != message.author:
        return [f"You can't check, {message.author.name}, because it's " f"{game.current_player.user.name}'s turn."]
...
def fold_hand(game: Game, message: discord.Message) -> List[str]:
    if game.state == GameState.NO_GAME:
        ...
        return ["You can't fold, because you're not playing, " f"{message.author.name}."]
    elif game.state == GameState.NO_HANDS:
        return ["You can't fold yet because the hands haven't been dealt yet."]
    elif game.current_player.user != message.author:
        return [f"You can't fold {message.author.name}, because it's " f"{game.current_player.name}'s turn."]
    else:
        return game.fold()
...
def set_option(game: Game, message: discord.Message) -> List[str]:
    tokens = message.content.split()
    if len(tokens) == 3:
        return ["You must specify a new value after the name of an option " "when using the `.p set` command."]
    elif len(tokens) == 2:
        return ["You must specify an option and value to set when using " "the `.p set` command."]
    elif tokens[2] not in GAME_OPTIONS:
        return [f"'{tokens[2]}' is not an option. Message `.p options` to see " "the list of options."]
    ...
    try:
        ...
        return [f"{tokens[2]} must be set to an integer, and '{tokens[3]}'" " is not a valid integer."]
...
def chip_count(game: Game, message: discord.Message) -> List[str]:
    if game.state in (GameState.NO_GAME, GameState.WAITING):
        return ["You can't request a chip count because the game " "hasn't started yet."]
...
def all_in(game: Game, message: discord.Message) -> List[str]:
    if game.state == GameState.NO_GAME:
        ...
        return ["You can't go all in, because you're not playing, " f"{message.author.name}."]
    elif game.state == GameState.NO_HANDS:
        return ["You can't go all in because the hands haven't " "been dealt yet."]
    elif game.current_player.user != message.author:
        return [f"You can't go all in, {message.author.name}, because " f"it's {game.current_player.user.name}'s turn."]
    else:
        return game.all_in()
...
commands: Dict[str, Command] = {
    ...
    'allin': Command('Bets the entirety of your remaining chips', all_in),
}
...
async def on_message(message: discord.Message):
    ...
    if message_split[0] == '.p':
        ...
        if len(message_split) <= 1:
            await message.channel.send("No command specified. " "Message `.p help` to see the list of commands.")
            return
        command = message_split[1]
        if command not in commands:
            await message.channel.send(
                f"{message.content} is not a valid command. " "Message `.p help` to see the list of commands."
            )
            return
